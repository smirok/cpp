...
Board::Board(int height, int width, int win_len) : _height(height),
                                                   _width(width),
                                                   _win_len(win_len),
                                                   _current_player(Players::O) {
    if (height <= 0 || width <= 0 || win_len <= 0)
        exit(0);
    if (_win_len > std::max(_height, _width))
        _win_len = std::max(_height, _width);
    ...
    for (int i = 0; i < _height; i++) {
        for (int j = 0; j < _width; j++) {
            ...
            for (int k = 0; k < _win_len; k++) {
                ...
                direction[2] &= (std::max(i, j) + k < std::min(_height, _width) &&
                                 _field[i + k][j + k] == _field[i][j]); // down-right
                direction[3] &= (j - k >= 0 && i + k < std::max(_height, _width) &&
...
std::vector<std::vector<FieldState >> Board::getField() const {
    return _field;
}
